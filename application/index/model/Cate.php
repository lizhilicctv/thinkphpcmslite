...
class Cate extends Model
{
    ...
    public function hot($id, $num, $offset, $order, $field, $where)
    {
        ...
        if (config('app_debug')) {
            $index_cate=Db::name('article')->where('isopen', 1)->where($where2)->where($where)->field($field)->limit($offset, $num)->order($order)->select();
        } else {
            if (!$index_cate=Cache::get('index_cate')) {
                $index_cate=Db::name('article')->where('isopen', 1)->where($where2)->where($where)->field($field)->limit($offset, $num)->order($order)->select();
                Cache::set('index_cate', $index_cate, 3600);
    ...
    public function hotimg($id, $num, $offset, $order, $field, $where)
    {
        ...
        if (config('app_debug')) {
            $index_cate=Db::name('article')->whereNotNull('pic')->where('isopen', 1)->where($where2)->where($where)->field($field)->limit($offset, $num)->order($order)->select();
        } else {
            if (!$index_cate=Cache::get('index_cate')) {
                $index_cate=Db::name('article')->whereNotNull('pic')->where('isopen', 1)->where($where2)->where($where)->field($field)->limit($offset, $num)->order($order)->select();
                Cache::set('index_cate', $index_cate, 3600);
